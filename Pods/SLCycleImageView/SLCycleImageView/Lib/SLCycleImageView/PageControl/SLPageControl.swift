...
//  TAPageControl
//
//  Created @objc @objc by Tanguy Aladenise on 2015-01-21.
...
@objc protocol SLPageControlDelegate : NSObjectProtocol {
    @objc optional func pageControl(_ pageControl: SLPageControl , didSelectPageAtIndex index: Int)
}
class SLPageControl : UIControl {
     ...
     */
    var dotViewClass: AnyClass? = SLAbstractDotView.self {
        didSet {
     ...
     */
    weak var delegate: SLPageControlDelegate?
    /**
...
// MARK: - Lifecycle
    ...
    func initialization() {
        self.dotViewClass = SLAnimatedDotView.self
...
// MARK: - Utils
    ...
    func generateDotView() -> UIView {
        ...
        if let dotViewClass = dotViewClass {
            ...
            if dotView is SLAnimatedDotView {
                (dotView as! SLAnimatedDotView).dotColor = dotColor
            }
        }
        else {
...
//        }
    ...
    func changeActivity(_ active: Bool, atIndex index: Int) {
        if self.dots.count > index, let abstractDotView = self.dots[index] as? SLAbstractDotView {
            
            if abstractDotView.responds(to: #selector(SLAbstractDotView.changeActivityState(_:))) {
                abstractDotView.changeActivityState(active)
            }
            else {
                print("Custom view : \(String(describing: self.dotViewClass)) must implement an 'changeActivityState' method or you can subclass \(SLAbstractDotView.self) to help you.")
            }
        }
