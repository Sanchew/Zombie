...
public extension ObservableType {
    func availableMap<R>(_ transform: @escaping (Self.E) throws -> R?) -> RxSwift.Observable<R> {
        return self.map(transform).filterNil()
    }
    
    func validMap<R: Occupiable>(_ transform: @escaping (Self.E) throws -> R) -> RxSwift.Observable<R> {
...
public extension ObservableType where E: Occupiable {
    func validMap<R>(_ transform: @escaping (Self.E) throws -> R) -> RxSwift.Observable<R> {
...
public extension ObservableType where E: Equatable {
    ...
    func filter(_ e: E) -> Observable<E> {
        return self.filter{ $0 == e }
    }
    
    func filterAndMapVoid(_ e: E) -> Observable<Void> {
        return self.filter(e).map{ _ in () }
    }
    
    func filter<R>(_ e: E, transform: @escaping (E) throws -> R) -> Observable<R> {
